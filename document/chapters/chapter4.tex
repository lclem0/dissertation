...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/camera-feed.png}
    \caption{RViz image viewer displaying the live camera feed from the Astra camera}
    \label{fig:camera-feed}
\end{figure}

\subsection{Unity Camera Feed Integration}

After the camera live feed was displayed in the Rviz simulation within the on-site environment, this data had to be transmitted to the Unity \ac{MR} application, allowing the remote user to observe the robot's environment in real-time, providing critical visual feedback necessary for effective remote collaboration.
