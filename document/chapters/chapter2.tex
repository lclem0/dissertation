...
\begin{introduction}
    This chapter provides an in-depth exploration of the core concepts supporting the shift toward Industry 5.0 and its application within Human-Robot Collaboration (\ac{HRC}). It begins with an overview of Industry 4.0 and its emphasis on advanced automation, followed by a discussion about Industry 5.0, which reintroduces the human element to create more collaborative and adaptive industrial environments. Furthermore, an examination on collaborative robots, Digital Twins (\ac{DT}), and Digital Realities (\ac{DR}), particularly Mixed Reality (\ac{MR}), as central technologies in achieving human-centered industrial systems. Additionally, relevant research on \ac{HRC} applications is reviewed, highlighting advancements, challenges, and future directions for integrating \ac{MR} and \ac{DT} in real-world manufacturing settings. Through these discussions, this chapter lays a conceptual foundation for understanding how these technologies can be implemented to create safe, effective, and seamless collaborative environments for both on-site and remote operators.
\end{introduction}
